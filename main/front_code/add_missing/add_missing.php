...
move_uploaded_file($missingTmpName,"..\..\..\admin\code\add_missing\upload\missing\\".$missing1);
...
/*$c = $con->prepare('SELECT * FROM missing WHERE SSN=:SSN');

$c->bindParam('SSN',$national_number);

$c->execute();

if ($c->rowCount() > 0) {
    echo '<div class="alert alert-success" role="alert">  هذا الرقم موجود مسبقا </div>';
}*/

 
    $insertw = $con->prepare("INSERT INTO missing(SSN,age,phone,First_Name,Mid_Name,Last_Name,relationWithMisssing,missing_date,height,weight,hair_color,skin_color,eye_color,governorate_code,city_code,nationality_code,department_code,disease_code,notes,image,added_by_citizines,gender) 
    VALUES(:ssn ,:age,:phone,:first_name,:mid_name,:last_name,:relation,:lshow,:height,:weight,:hair_color,:skin_color,:eye_color,:governate,:city,:nation,:department,:disease,:desc,:img,:added_by,:gender)");
    ...
    $insertw->bindparam("added_by",$_SESSION['ssn']);
